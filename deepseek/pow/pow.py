...
class DeepSeekWASMHasher:
    ...
    def solve(self, challenge: str, salt: str, difficulty: int, expire_at: int) -> int:
        ...
        try:
            challenge_ptr, challenge_len = self.write_to_memory(challenge)
            prefix_ptr, prefix_len = self.write_to_memory(prefix)
...
def get_hasher() -> DeepSeekWASMHasher:
    global hasher_instance
    if hasher_instance is None:
        hasher_instance = DeepSeekWASMHasher().init(WASM_PATH)
